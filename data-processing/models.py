...
# 5. Tabella TEAM PERFORMANCE (Contesto Squadra: ELO + Forma)
class TeamPerformance(Base):
    ...
    season = Column(String, nullable=False)  # es. "2425"
    
    # ELO Rating System
    elo = Column(Float, default=1500.0)  # Rating ELO (1500 = media)
    
    # Rolling Form (ultimi 5 match)
    rolling_xg_form = Column(Float)  # Media xG fatti ultimi 5 match
    rolling_ga_form = Column(Float)  # Media gol subiti ultimi 5 match
